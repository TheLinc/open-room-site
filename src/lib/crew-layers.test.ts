...
describe("layersOf", () => {
  test.each(crewSprites.map((s) => [s.id, s] as const))(
    "%s has a screen, a body and two hands",
    (_, sprite) => {
      ...
      for (const c of l.handL)
        expect(c.x).toBeLessThan(l.screen.x + l.screen.w / 2);
      for (const c of l.handR)
        expect(c.x).toBeGreaterThan(l.screen.x + l.screen.w / 2);
    },
  );
